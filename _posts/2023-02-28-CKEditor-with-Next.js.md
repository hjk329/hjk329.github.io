...
우리 프로젝트는 Next.js를 기반으로 TypeScript를 사용한다.  
CMS를 개발하면서 에디터가 필요해서 [`React-Quill(이하 리액트 퀼)`](https://github.com/zenoamaro/react-quill)을 설치했었다.    

간단하게 글 작성 및 편집을 할 수 있으면서 Next.js와의 호환성도 좋고, 타입스크립트도 지원되어서 아주 편리하게 사용 중이었다.  
<br>


이후 CMS 고도화가 필요해지면서 리액트 퀼에서 기본적으로 제공하지 않는 추가적인 기능들이 요구되었다.  
예를 들어서, 구분 선을 보여준다거나 테이블, 이미지 업로드 시 자동으로 캡션이 추가되는 기능 등이었다.  
<br>


처음에는 객기(?)로 모든 기능들을 커스터마이징해서 개발해보려고 했다. 🐶 🐾   
그러다 보니 속도가 더뎠고, 에디터 개발 그 자체가 태스크가 되어버린 듯했다.  
<br>


사실 이번 스프린트에서 에디터를 개발하는 목적은 개발자를 통해서 JSON으로 관리되던 콘텐츠를, 개발자가 아닌 팀원분들도 에디터를 사용해서 직접 발행하도록 하는 것이었다.     
즉, 우리만의 에디터 자체를 개발하는 것이 스프린트의 주된 목적은 아니었다.  
...
이에 따라 에디터에 대한 요구 사항을 가장 빠르고 편리하게 구현할 수 있는 다른 라이브러리를 찾기 시작했다.  
...
데모를 사용해보며 우리에게 필요한 기능들이 기본적으로 제공되는 점을 확인했다.  
...
(두괄식 커뮤니케이션이 좋다는데 이제는 정말 시작해볼게요 😇)

<br><br>


CKEditor는 Next.js를 공식적으로 지원하고 있지는 않다.  
만약 npm으로 CKEditor 패키지를 설치하고, 플러그인을 사용하면 이런 이슈가 발생할 수 있다.  
...
[`Read more`](https://nextjs.org/docs/messages/css-npm) 를 클릭해보면 메인테이너에게 요청해보라는 내용이 해결책중 하나로 제시된다 ㄴㅇㄱ (대충 '상상도 못한' 짤을 형상화)  
...
`app` 디렉토리는 베타 버전이라서 그 안정성을 확신할 수 없을 뿐더러, 우리 프로젝트는 아직 Next.js를 버전 13으로 업그레이드하지 않은 상황이라서 해결책이 되지 않았다.  
...
문서에 의하면 [Online Builder(이하 온라인 빌더)](https://ckeditor.com/ckeditor-5/online-builder/)로 툴바를 커스텀할 수 있다.   
내가 원하는 플러그인을 모두 선택한 후에 해당 에디터의 빌드 파일 (zip파일)을 직접 다운로드하고 프로젝트로 옮겨서 설치하는 방식이었다.  

<br>


나는 아주 의욕적으로(복선) 거의 모든 플러그인을 선택한 채로 zip 파일을 내려 받았다. 😶‍🌫️   
플러그인을 임포트하면 위 오류가 발생하니까 더 추가할 플러그인이 없을 정도로 꽉꽉 채운 에디터를 설치하고 싶었기 때문이다 👉👈
...
CKEditor의 공식 문서[Integrating a build from the online builder
](https://ckeditor.com/docs/ckeditor5/latest/installation/frameworks/react.html#integrating-a-build-from-the-online-builder)에 따라서 진행했다.  
...
위 명령어를 터미널에 그대로 복붙한 결과는 아래와 같았다.  
...
저 상태에서 터미널에 `cd ./ckeditor5` 를 입력하면 ckeditor5로 정상적으로 이동되었다.  
...
우여곡절 끝에 설치한 에디터 모듈을 찾지 못하는 것 같았다. 🤦🏻‍♀️  
