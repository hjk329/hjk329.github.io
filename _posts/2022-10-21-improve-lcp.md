...
- 페이지의 콘텐츠가 렌더링되기 위해서 추가적인 자바스크립트 요청이 완료되기를 요구하지 않는다.


SSR의 이점은 서버에서 처리할 추가 시간만 요구한다는 것이다. (이는 TTFB를 늦출 수 있다.) <br>
이러한 트레이드 오프(TTFB를 늦추는 대신에 리소스 렌더링 타임을 줄이는 것)는 서버에서의 처리 시간은 제어할 수 있는 반면에 사용자의 네트워크 및 장치 기능은 그렇지 않다는 점에서 대개 가치가 충분하다. 
<br>
...
CDN을 이용해서 리소스가 이동해야하는 물리적인 거리를 줄이는 방법이 있다. <br>
CDN을 이용하면 일반적으로 리소스의 크기를 줄여 이전의 모든 크기 축소 권장 사항을 자동으로 구현할 수 있으므로 좋은 선택이다. <br>
...
#### 네트워크 타임을 완전히 제거한다
효율적인 캐시 정책으로 정적 리소스들을 제공한다면 캐시에서 가져온 리소스를 보여줄 것이므로 리소스 로드 타임을 0으로 만들 수가 있다. <br>
...
### 정리
LCP는 속도는 많은 요소에 의해 영향을 받을 수 있으므로, 이를 개선하면 로드와 관련된 다른 메트릭들도 향상될 수 있다. <br>

크게 볼때 LCP를 최적화하는 방안은 아래와 같다.
