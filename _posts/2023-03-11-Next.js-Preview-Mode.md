...
다시 말해 `http://localhost:3000/api/preview-clear` 로 위 API를 호출하면 `__prerender_bypass`, `__next_preview_data` 를 포함한 쿠키가 제거된다.   
...
빌드 타임 이후에 게시물을 수정하거나, 새 글을 발행하기 전에 `미리 보기`를 해보자!  


- 빌드 타임 이후에 수정한 게시물을 발행 전에 미리 보기
`$ yarn build` -> `$ yarn start` 명령어를 순서대로 실행하여 빌드한 후에 프로덕션 환경을 띄웠다.  
...
블로그 리스트 페이지에서 아무리 새로 고침을 해도 새로 추가된 게시물은 반영되지 않는다.  
...
또한, 블로그 해커톤을 통해서 완전 집중해서 공부하고, 공부한 내용을 정리해볼 수 있어서 참 좋았다! ✨😇👏   
