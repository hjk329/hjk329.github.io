...
브라우저에서 확인해보자! 👀
1) 버튼을 클릭하면 alert이 뜬다.
2) Clicked: true 로 변경이 된다.
3) URL이 `'/my-page/info?status=editing'` 로 변경되면서 clicked 값이 false 로 다시 변경된다.
...
1) 버튼을 클릭하면 alert이 뜬다.
2) Clicked: true 로 변경이 된다.
3) URL이 `'/my-page/info?status=editing'` 로 변경되어도 clicked  true 로 유지된다.
